...
AS
	INSERT INTO [User] VALUES(@id, @name, @email, @passwordHash)
RETURN 0
GO
...
AS
	SELECT @id = [User].Id, @email = [User].Email, @name = [User].Name FROM [User]
...
BEGIN
    UPDATE [User]
    SET Id = @id, Email=ISNULL(@email,Email), Name=ISNULL(@name,Name), PasswordHash=ISNULL(@passwordHash, PasswordHash)
    WHERE id=@id
END
GO
